...
function nblogpdf(r, p, k)
    ...
    seq_depth = SEQ_DEPTH
    r * (log(p_mod)-log(seq_depth+p_mod-seq_depth*p_mod)) + k * (log(seq_depth-seq_depth*p_mod)-log(seq_depth+p_mod-seq_depth*p_mod)) - log(k + r) - logbeta(r, k + 1)
...
function mix_nbpdf_2(rr::AbstractVector, pp::AbstractVector,
    ww::AbstractVector, k)
    ret = ww[1] .* nbpdf.(rr[1], pp[1], k)
