...
function pred_pdf_infe(model, x::AbstractVector, yy)
    rr, pp, ww = model(x)
...
function loss_crossentropy(x::AbstractVector, y::AbstractVector,z::AbstractVector, model)
    pred = pred_pdf(model, x, 0:length(y)-1)
...
m4(ŷ) = sum(((0:length(ŷ)-1) .-mean_val(ŷ)) .^4 .* ŷ)

sk_val(ŷ) =  m3(ŷ) ./ sqrt(var_val(ŷ)).^3+1
kur_val(ŷ) =  m4(ŷ) ./ var_val(ŷ).^2
bio_val(ŷ) = ((sk_val(ŷ).-1).^2 .+1) ./ kur_val(ŷ)

mean_loss(ŷ, y) = (log10(mean_val(ŷ)/y.+0.0001))^2 +0.00001
fano_loss(ŷ, y) = (log10((var_val(ŷ)/mean_val(ŷ))/y.+0.0001))^2 +0.00001
sk_loss(ŷ, y) = (log10(abs(sk_val(ŷ)/y.+0.0001)))^2 +0.00001
...
function loss_crossentropy_statical(x::AbstractVector, y::AbstractVector,z::AbstractVector, model)
    ...
    Flux.crossentropy(pred, y)+(mean_loss(pred,z[1])+fano_loss(pred,z[3])+kur_loss(pred,z[5])+bio_loss(pred,z[6]))*LAMBDA
...
"""
function batch_loss(X::AbstractVector, y::AbstractVector,z::AbstractVector, model;
                    loss = loss_crossentropy)
    ret = loss(X[1], y[1],z[1], model)
    
    @inbounds for i in 2:length(X)
        ret += loss(X[i], y[i],z[i], model)
    end
...
"""
  function train_round!(trainer::NNTrainer,threads::Bool=true; loss=loss_cross_entropy)
    ...
    for (x, y,z) in trainer.train_loader
        @maybe_threaded for i in 1:nt
            grads[i] = Flux.gradient(p) do
                batch_loss((@view x[i:nt:end]), (@view y[i:nt:end]),(@view z[i:nt:end]), trainer.model; loss)
            end
        end
