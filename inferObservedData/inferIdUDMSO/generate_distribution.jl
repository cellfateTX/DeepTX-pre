...
result_dir = joinpath(RESULT_DIR, "IduDMSO/")
idu_data_path = joinpath(result_dir,"idu_estimated_$MODEL_TYPE.csv")
idu_save_path =joinpath(result_dir, "iduSSADistribution/")
estimated_df = DataFrame(CSV.File(joinpath(idu_data_path)))[:,1:5]
simulatin_es_parameters(estimated_df,idu_save_path,SEQ_DEPTH)

dmso_data_path = joinpath(result_dir,"dmso_estimated_$MODEL_TYPE.csv")
