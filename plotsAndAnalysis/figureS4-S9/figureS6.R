...
doseZero_kl_df = read.csv( "result/Ten5FU/doseZero_kl.csv", row.names =2) 
gene_estimated_matrix_doseZero = read.csv(file = "result/Ten5FU/doseZero_estimated_model_stats_prob.csv", row.names =6)
...
gene_estimated_matrix_idu = read.csv(file = "result/Ten5FU/doseZeroSSADistribution/distribution_2.csv")
...
P9_C <- ggplot(kl_gene_estimated_matrix_doseFifty, aes(mean_true, fill = as.factor(kl_list))) +
  geom_histogram(bins = 30) + 
  scale_fill_manual(values = c( "#71A1C6","#E26463")) +
...
genes_name = names(RNA_histogram_DATA)
gene_estimated_matrix_Fifty = read.csv(file = "result/Ten5FU/doseTenSSADistribution/distribution_1.csv")
P9_D = plot_distribution(RNA_histogram_DATA,gene_estimated_matrix_Fifty,genes_name[1])

gene_estimated_matrix_Fifty = read.csv(file = "result/Ten5FU/doseTenSSADistribution/distribution_2.csv")
P9_E = plot_distribution(RNA_histogram_DATA,gene_estimated_matrix_Fifty,genes_name[2])

gene_estimated_matrix_Fifty = read.csv(file = "result/Ten5FU/doseTenSSADistribution/distribution_3.csv")
...
figure9
ggsave(sprintf( '%sfigureS6.pdf',save_figure_dir),width = 6,height = 7.2,useDingbats = FALSE)
