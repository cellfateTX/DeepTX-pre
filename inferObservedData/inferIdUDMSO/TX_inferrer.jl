...
gene_exp_idu = DataFrame(
    CSV.File(
        joinpath(DATA_DIR,"IduDMSO/IdU_norm_filter.csv" ),
    ),
)[!,1:10]
gene_exp_dmso = DataFrame(
    CSV.File(
        joinpath(DATA_DIR,"IduDMSO/DMSO_norm_filter.csv" ),
    ),
)[!,1:10]
...
es_file_name_idu = "IduDMSO/idu_estimated_$MODEL_TYPE.csv"
es_file_name_dmso = "IduDMSO/dmso_estimated_$MODEL_TYPE.csv"
