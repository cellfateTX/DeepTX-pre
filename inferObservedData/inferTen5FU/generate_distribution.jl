...
idu_save_path =joinpath(RESULT_DIR, "doseTenSSADistribution/")

result_dir = joinpath(RESULT_DIR, "Ten5FU/")
idu_data_path = joinpath(result_dir,"doseTen_estimated_$MODEL_TYPE.csv")
idu_save_path =joinpath(result_dir, "doseTenSSADistribution/")
estimated_df = DataFrame(CSV.File(joinpath(idu_data_path)))[:,1:5]
simulatin_es_parameters(estimated_df,idu_save_path,SEQ_DEPTH)

dmso_data_path = joinpath(result_dir,"doseZero_estimated_$MODEL_TYPE.csv")
